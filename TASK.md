...
검증 근거를 보존하는 원본 카탈로그는 **100개·2,700조합**이다. 실제 뽑기에는 한국의 큰 생활 행동군이 설명 없이 알아볼 수 있는 **26개·702조합**만 사용한다.

## 현재 상태

- 근거 카탈로그에는 원본 100개·2,700조합이 그대로 보존돼 있다.
- 실제 앱 뽑기는 한국 대중 소비자의 반복 생활 문제를 다루는 원본 26개·702조합만 사용한다.
- 2026-07-19 한국 수요 선별은 특정 직무·해외 플랫폼·전문 파일 지식이 필요한 원본을 먼저 제외하고, 문제의 폭·반복성·국내 적합성·결과 이해성을 교차검토했다. 해외 매출은 문제 존재 근거로만 사용했다.
- 통과 원본 26개의 제품명을 `맛핀`, `한입코치`, `곁불`, `마음수호신`처럼 장면·감정·캐릭터가 기억되는 이름으로 바꾸고, 한 줄 설명에는 입력과 결과를 중학생도 이해하도록 남겼다.
...
## 제품 여정 작업 최종 결과 — 2026-07-16

- 데이터 확장 완료 상태인 앱 원본 100개·런타임 조합 2,700개·리서치 판정 8,406개는 그대로 유지했다.
- T24~T28을 구현했다: 최소 3초 읽기 대기와 Apple식 상단 dot/pause 인디케이터·가이드 패널 순서·단계별 헤더·실제 반응 기반 수정·원본/수정본 버전 분리·중복 없는 제품 이벤트·최종 반응형/접근성 점검.
- 단위 테스트 75/75, 관련 E2E 38/38, 연구 검증 5종, 프로덕션 빌드, 인앱 브라우저 6개 뷰포트 검증을 통과했다.
