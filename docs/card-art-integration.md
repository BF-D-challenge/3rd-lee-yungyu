...
> 원칙: 일러스트는 수트의 무드, **카투슈의 이름**이 구체 값. 경로는 [`lib/card-art.ts`](../lib/card-art.ts), 렌더는 [`components/organisms/slot/tarot-card.tsx`](../components/organisms/slot/tarot-card.tsx).
