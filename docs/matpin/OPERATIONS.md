...
맛핀 CRM 구현과 요청된 문서 아카이브는 각각 `c20c1f2`, `6f18328`로 커밋해 원격 `codex/matpin-conversation-crm` 브랜치에 올렸습니다. CRM 검토 단위는 Draft PR #30입니다. CI Ubuntu에서만 실패한 Idea Lab의 고정 높이 검사를 실제 내용 잘림 검사로 바꿨고, 후속 GitHub CI 재실행도 성공했습니다. 이 업로드와 CI 성공은 운영 배포, 운영 DB 전환 또는 관리자 로그인 성공을 뜻하지 않습니다.

2026-08-11 출시 통합 점검에서는 최신 `main`, Draft PR #30의 CRM, 전 세계 장소 검색과 CTA `#how` 복구를 별도 로컬 출시 브랜치에 합쳤습니다. `npm run typecheck`, `npm run lint`, `npm run build`, 단위 테스트 71개 파일의 568개 테스트와 Playwright 131개 테스트가 모두 통과했어요. 운영 Supabase를 읽기 전용으로 확인한 결과 메시지는 `saved` 9건뿐이었고, 분석 큐 0건, 관리자 pending action 0건, 처리 중 캐시 0건이었습니다. 준비된 캐시 2건은 전 세계 장소 계약 버전 마이그레이션에서 무효화될 대상입니다. 운영 DB 마이그레이션과 배포는 아직 하지 않았으며, 아래 maintenance 전환 순서를 지키기 전까지 상태는 계속 HOLD예요.
