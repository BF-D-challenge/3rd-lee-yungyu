...
Meta 조회 시간 예산, 불확실한 발송의 중복 차단과 자동 답장 1,000바이트 제한의 수정 및 회귀 테스트는 완료했습니다. 관리자 감사 로그의 보존 기간과 계정 삭제 시 처리 정책은 배포 전에 결정하고, 자동 삭제나 계정 삭제 연동이 필요하면 같은 정책을 코드와 DB에 반영해요.
...
- 관리자 감사 로그의 보존 기간과 계정 삭제 시 처리 정책 확정
