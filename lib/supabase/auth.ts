// 구글 OAuth (Supabase Auth). 발행(S3) 순간에만 요구 — 투표 수신자에겐 절대 로그인 없음.
//   Supabase+Google provider가 설정돼 있으면 실제 OAuth, 아니면 호출부가 가짜 통과로 폴백.
...
/** 실제 구글 로그인 활성 여부. false면 발행 화면이 가짜 통과로 폴백. */
