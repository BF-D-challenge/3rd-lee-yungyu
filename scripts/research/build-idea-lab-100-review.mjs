...
<body>
  <main>
    <header>
      ...
      <p class="scope-note"><strong>읽는 법:</strong> 2,700개는 100개 원본에서 만든 검토 조합 수이지 독립 원본 수가 아닙니다. 캐릭터 챗 전용 제품은 이번 최종 100개에 넣지 않았습니다. 조사풀의 직접 사례는 범용 AI 한 번으로 대체되거나 다른 참여자가 필요한 구조라 최종 원본 기준을 통과하지 못했습니다.</p>
    </header>
    ...
    <section id="grid" class="grid" aria-label="원본 아이디어 목록"></section>
  ...
  <script>
    ...
    const list = (items, render) => items.map((item) => "<li>" + render(item) + "</li>").join("");
    const renderCard = (item) =>
      '<article data-search="' + escapeHtml(searchableText(item)) + '" data-platform="' + escapeHtml(item.source.platform) + '">' +
        ...
        '<div class="sections"><section class="section payer"><h3>돈 낼 사람 · 3</h3><ol>' + list(item.payers, (entry) => '<strong>' + escapeHtml(entry.value) + '</strong><span>' + escapeHtml(entry.detail) + '</span>') + '</ol></section>' +
        '<section class="section moment"><h3>필요한 순간 · 3</h3><ol>' + list(item.moments, (entry) => '<strong>' + escapeHtml(entry.value) + '</strong><span>' + escapeHtml(entry.detail) + '</span>') + '</ol></section>' +
        '<section class="section twist"><h3>한 끗 변화 · 3</h3><ol>' + list(item.twists, (entry) => '<strong>' + escapeHtml(entry.resultTitle) + '</strong><span>' + escapeHtml(entry.value) + '<br>' + escapeHtml(entry.smallestBuild) + '</span>') + '</ol></section></div></article>';
...
console.log(JSON.stringify({ outputPath, scenarios: scenarios.length, bytes: Buffer.byteLength(html) }, null, 2));
