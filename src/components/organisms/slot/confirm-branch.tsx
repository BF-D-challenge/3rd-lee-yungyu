...
import { duelUrl, shareUrl, toPayload, type CardPayload } from "@/lib/share";
import { track } from "@/lib/track";
import { copyText } from "./copy";
import { IdeaFlipCard } from "./idea-flip-card";

/** 원본 vs 방금 만든 새 카드를 잇는 듀얼 공유 체인. vote-panel.tsx와 리터럴 일치시킬 것 */
...
export function ConfirmBranch({ open, combo, onClose }: ConfirmBranchProps) {
  ...
  const onShare = async (): Promise<boolean> => {
    let url = shareUrl(toPayload(combo));
    let isDuel = false;
    try {
      const raw = sessionStorage.getItem(ORIGIN_PAYLOAD_KEY);
      if (raw) {
        const origin = JSON.parse(raw) as CardPayload;
        url = duelUrl(origin, toPayload(combo));
        isDuel = true;
        sessionStorage.removeItem(ORIGIN_PAYLOAD_KEY); // 1회성 — 다음 확정엔 영향 없음
      }
    } catch {
      /* 파싱 실패 시 일반 단일 카드 공유로 폴백 */
    }
    const ok = await copyText(url);
    if (ok) track("confirm_share_click", { seed_tag: combo.seed.id, is_duel: isDuel });
    return ok;
  };
