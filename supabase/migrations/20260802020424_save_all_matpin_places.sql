alter table public.matpin_saved_places
  drop constraint if exists matpin_saved_places_message_id_key;

alter table public.matpin_saved_places
  drop constraint if exists matpin_saved_places_sender_hash_reel_id_key;

create index if not exists matpin_saved_places_message_id_idx
  on public.matpin_saved_places (message_id);

create unique index if not exists matpin_saved_places_active_place_unique_idx
  on public.matpin_saved_places (sender_hash, reel_id, ((place ->> 'id')))
...
declare
  ...
  v_saved_id bigint;
begin
  perform public.matpin_save_places(
    p_message_id,
    p_sender_hash,
    jsonb_build_array(p_place),
    p_confirmation_source
  );

  select reel_id into v_reel_id
  from public.matpin_instagram_messages
  where id = p_message_id and sender_hash = p_sender_hash;
  ...
  where sender_hash = p_sender_hash
    and reel_id = v_reel_id
    and place ->> 'id' = p_place ->> 'id'
...
revoke all on function public.matpin_confirm_place(uuid, text, jsonb, text)
  from public, anon, authenticated;

grant execute on function public.matpin_save_places(uuid, text, jsonb, text)
  to service_role;
grant execute on function public.matpin_confirm_place(uuid, text, jsonb, text)
  to service_role;
