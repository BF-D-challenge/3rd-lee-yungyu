...
test("로그인 스플래시는 인증 전 카드 뽑기를 막고 인증 후 메인 앱을 연다", async ({ page }) => {
  ...
  await expect(fakeCursor).toHaveAttribute("data-step-px", "1");
  await expect(fakeCursor).toHaveAttribute("data-easing", "ease-in-out");
  await expect(fakeCursor).toHaveCSS("transition-property", "opacity");
  ...
  const titleBounds = await page.locator("#creator-login-title").boundingBox();
  ...
  expect(previewBounds!.y).toBeLessThanOrEqual(2);
  expect(Math.abs(titleBounds!.x + titleBounds!.width / 2 - 160)).toBeLessThanOrEqual(1);
